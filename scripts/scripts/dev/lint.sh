...
RUFF_BACKEND=0
if [ "$FIX" = true ]; then
    "$PYTHON" -m ruff check "$BACKEND_DIR/apps" --fix 2>&1 || RUFF_BACKEND=$?
else
    "$PYTHON" -m ruff check "$BACKEND_DIR/apps" 2>&1 || RUFF_BACKEND=$?
fi
[ $RUFF_BACKEND -eq 0 ] && ok "ruff: backend clean" || { fail "ruff: backend has issues"; RESULT=1; }

log "Running ruff on code/ai_services..."
RUFF_AI=0
if [ "$FIX" = true ]; then
    "$PYTHON" -m ruff check "$AI_DIR" --fix 2>&1 || RUFF_AI=$?
else
    "$PYTHON" -m ruff check "$AI_DIR" 2>&1 || RUFF_AI=$?
fi
[ $RUFF_AI -eq 0 ] && ok "ruff: ai_services clean" || { fail "ruff: ai_services has issues"; RESULT=1; }
...
if [ "$FIX" = true ]; then
    ...
    "$PYTHON" -m black --check "$BACKEND_DIR/apps" "$AI_DIR" 2>&1 || BLACK_RESULT=$?
    [ $BLACK_RESULT -eq 0 ] && ok "black: no formatting issues" || { fail "black: formatting issues found (run with --fix to auto-format)"; RESULT=1; }
fi
